...
BOARD_FILENAME = GAME_DATA_DIR + 'RatRaceBoardSpaces.json'
SMALL_DEAL_CARDS = GAME_DATA_DIR + 'SmallDealCards.json'
BIG_DEAL_CARDS = GAME_DATA_DIR + 'BigDealCards.json'
DOODAD_CARDS = GAME_DATA_DIR + 'DoodadCards.json'
MARKET_CARDS = GAME_DATA_DIR + 'MarketCards.json'


def cash_flow_rat_race_simulate_game(a_profession,
                                     a_strategy,
                                     verbose=False):
    """Initiate Cash Flow Rat Race Simulation."""
    # Create Rat Race Board
    rat_race_board = board.load_board_spaces(BOARD_FILENAME)

    # Create 4 Card Decks - not used except as starting points
    small_deal_card_deck_master = cards.load_all_small_deal_cards(
        SMALL_DEAL_CARDS)
    big_deal_card_deck_master = cards.load_all_big_deal_cards(BIG_DEAL_CARDS)
    doodad_card_deckMaster = cards.load_all_doodad_cards(DOODAD_CARDS)
    market_card_deck_master = cards.load_all_market_cards(MARKET_CARDS)
...
if __name__ == '__main__':
    import unittest
    unittest.main(module='cash_flow_rat_race_simulate_game_test', verbosity=2)

...
def CashFlowRatRaceGameSimulation(aProfession,
                                  aStrategy,
                                  verbose = False):
    ratRaceBoard = board.getBoardSpaces("RatRaceBoardSpaces.json")    #Create Rat Race Board
    ...
    #Start Main Loop, initiate decks to be used, shuffle and create turn counter
    ...
    #amIRich, amIBroke = aPlayer.refresh()
    ...
    while True:
        ...
        if player_choice.chooseToPayOffLoan(playerOnBoard[0]):
            amIRich, amIBroke = playerOnBoard[0].refresh()
            if amIRich:
                print("After", turnCounter, "turns, Player", playerOnBoard[0].getName(), "is rich and wins")
                ...
                print("Player", playerOnBoard[0].getName(), "is broke and looses")
        ...
        if playerOnBoard[0].getCharityTurns() > 0:
            ...
            noOfDice = player_choice.chooseNoDie([1, 2], playerOnBoard[0].getStrategy(), verbose)
        else:
            noOfDice = 1
        ...
        if playerOnBoard[0].getSkippedTurnsRemaining() > 0:
            if verbose:
                print("Using a layoff day, " + str(playerOnBoard[0].getSkippedTurnsRemaining()) + " turns remaining")
            playerOnBoard[0].useLayoff()
            continue

        #Roll the die
        aDieRoll = dieRoll.rollDie(playerOnBoard[0].getStrategy(), noOfDice, verbose)

        #Move based on dice roll
        playerOnBoard[1], passedPaycheck, newBoardSpace = ratRaceBoard.movePlayerBoardSpaces(playerOnBoard, aDieRoll)
        ...
        boardSpaceAction.boardSpaceAction(playerOnBoard, newBoardSpace, verbose,
        ...
        if amIRich:
            ...
            if verbose:
                print("After", turnCounter, "turns, Player", playerOnBoard[0].getName(), "is broke and looses")
                print(playerOnBoard[0])
                print("Sold Assets\n\n", playerOnBoard[0].getSoldAssets())
            break
...
if __name__ == '__main__':
    ...
    #Example settings to test
        ...
        amIRich, amIBroke, turnCounter = CashFlowRatRaceGameSimulation(professionDict[professionName],

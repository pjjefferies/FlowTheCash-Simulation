...
def singleRunRepeat(professionObj, strategyObj, repetitions, verbose):
    ...
    for test in range(0,repetitions):
        ...
        amIRich, amIBroke, turnCounter = cfrrs.CashFlowRatRaceGameSimulation(professionObj,
                                                                             strategyObj,
                                                                             verbose)
...
if __name__ == '__main__':
    ...
    resultsTitleList = ['test no.', 'professionName', 'strategyName', 'Am I Rich', 'Am I Poor', 'Turns']
    resultsListToSave = []

    gameFileLogFilename = "GameLog-" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S") + ".csv"
    with open(gameFileLogFilename, "w", newline='') as OutputFile:
        ...
        for professionToEval in iter(professionDict):
            for strategyToEval in iter(strategyDict):
                resultsListToSave = singleRunRepeat(professionDict[professionToEval],
                ...
                if timeSim:
                    ...
                    print("Time per prof, strat combo:", "{0:.1f}".format(singleTime), "seconds.\nTotal Expected Time:", "{0:.1f}".format((singleTime*noSims)))
                    timeSim = False
        OutputFile.close()

    print("Time to run", testsToRun, "tests for each profession/strategy:", '{0:.2f}'.format(time.time() - startTime), "seconds.")